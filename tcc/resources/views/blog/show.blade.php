<x-app-layout :title="$post->titulo">
    <article class="col-span-4 md:col-span-3 mt-10 mx-auto py-5 w-full" style="max-width:700px">
        <img class="w-full my-2 rounded-lg" src="{{ $post->getThumbnailUrl() }}" alt="Capa">
        <h1 class="text-4xl font-bold text-left text-gray-800">
            {{ $post->titulo }}
        </h1>
        <div class="mt-2 flex justify-between items-center">
            <div class="flex py-5 text-base items-center">
                <x-post.author :author="$post->author" size="lg" />
                <span class="text-gray-500 text-sm"> | Tempo de leitura: {{ $post->getReadingTime() }} minuto</span>
            </div>
            <div class="flex items-center">
                <span class="text-gray-500 mr-2"> {{ $post->publidado_em->diffForHumans() }}</span>
        ...
        <livewire:post-comentarios :key="'comentario'.$post->id" :$post />
